import{n as u,t as c,_ as x,c as y}from"./index-DEW-c385.js";let n=1;const e={nome:"",email:"",telefone:"",senha:"",bio:"",nicho:null,subcategorias:[],raio:20,dias_semana:[],horarios:[],avatarFile:null,avatarPreview:null,docFile:null,selfieFile:null},v=[{emoji:"🧱",nome:"Pedreiro",tag:"Construção / Reforma",subs:["Geral","Acabamento","Alvenaria","Revestimento"]},{emoji:"⚡",nome:"Eletricista",tag:"Instalação / Manutenção",subs:["Residencial","Predial","Industrial","Automação"]},{emoji:"🔧",nome:"Encanador / Hidráulica",tag:"Reparos Hidráulicos",subs:["Reparo de Vazamentos","Instalação Nova","Desentupimento"]},{emoji:"🖌️",nome:"Pintor",tag:"Pintura",subs:["Residencial","Comercial","Texturização","Gesso / Drywall"]},{emoji:"🧹",nome:"Limpeza / Diarista",tag:"Faxina",subs:["Pontual","Semanal","Quinzenal","Mensal"]},{emoji:"🌿",nome:"Jardineiro / Paisagista",tag:"Paisagismo",subs:["Manutenção","Poda","Plantio","Irrigação"]},{emoji:"🪵",nome:"Marceneiro / Carpinteiro",tag:"Móveis / Madeira",subs:["Móveis Planejados","Reparo","Deck","Portas / Janelas"]},{emoji:"🛡️",nome:"Vigilante / Segurança",tag:"Proteção",subs:["Diurno","Noturno","12x36","Portaria"]},{emoji:"🚗",nome:"Motorista / Entregador",tag:"Transporte",subs:["Moto","Carro","Van","Caminhão"]},{emoji:"🤲",nome:"Cuidador de Idosos / Crianças",tag:"Cuidados Pessoais",subs:["Idoso","Criança","PCD","Hospitalar"]},{emoji:"🏭",nome:"Auxiliar de Produção",tag:"Indústria / Logística",subs:["Alimentício","Montagem","Embalagem","Estoque"]},{emoji:"🏗️",nome:"Aux. Serviços Gerais",tag:"Apoio Operacional",subs:["Condomínio","Escritório","Obra","Shopping"]},{emoji:"🍳",nome:"Cozinheiro / Auxiliar de Coz.",tag:"Gastronomia",subs:["Eventos","Fixo","Diarista","Fast Food"]},{emoji:"💻",nome:"Técnico de TI / Informática",tag:"Suporte Técnico",subs:["Manutenção de PC","Redes","CFTV","Software"]},{emoji:"💆",nome:"Esteticista / Massagista",tag:"Beleza e Bem-estar",subs:["Massagem","Depilação","Estética Facial","Unhas"]},{emoji:"➕",nome:"Outros",tag:"Personalizado",subs:["Digitar manual..."]}];function F(l){n=1,d(l)}function d(l){l.innerHTML=`
    <div style="height:100%; background:#F4F5F7; display:flex; flex-direction:column; padding:60px 32px 40px;">
      ...
      <div style="display:flex; gap:8px; margin-bottom:28px;">
        ${[1,2,3,4].map(i=>`
          <div style="flex:1; height:5px; border-radius:3px; background:${i<=n?"#4F46E5":"#E5E7EB"};"></div>
        `).join("")}
      </div>
      ...
      <div style="margin-top:auto; padding-top:24px; background:linear-gradient(to top, #F4F5F7 80%, transparent); margin: 0 -32px; padding: 24px 32px 0;">
        ...
        ">${n===4?"Enviar e Criar Conta":"Continuar"}</button>
      </div>
    </div>
  `;const m=document.getElementById("etapaContent");if(n===1){m.innerHTML=`
    ...
    `;const i=document.getElementById("btnFotoReg"),o=document.getElementById("inpFotoReg");i&&o&&(i.onclick=()=>o.click(),o.onchange=t=>{const r=t.target.files[0];r&&(e.avatarFile=r,e.avatarPreview=URL.createObjectURL(r),i.innerHTML=`<img src="${e.avatarPreview}" style="width:100%;height:100%;object-fit:cover;">`)});const a=document.getElementById("btnGerarIA");a&&(a.onclick=()=>{const t=document.getElementById("nome").value.trim()||"Profissional",r=document.getElementById("bio");a.innerHTML="⏳ Gerando...",setTimeout(()=>{r.value=`Olá! Meu nome é ${t} e sou um profissional dedicado e detalhista, sempre focado em entregar o melhor resultado para meus clientes. Tenho experiência prática e busco sempre aprimorar minhas habilidades. Estou à disposição para ajudar no que for preciso!`,a.innerHTML="✨ Gerar com IA"},1200)})}else if(n===2)m.innerHTML=`
    ...
    `,document.querySelectorAll(".niche-header").forEach(i=>{i.onclick=()=>{const o=i.closest(".niche-card").dataset.nicho;e.nicho!==o&&(e.nicho=o,e.subcategorias=[],d(l))}}),document.querySelectorAll(".spec-chip").forEach(i=>{i.onclick=o=>{o.stopPropagation();const a=i.dataset.sub;e.subcategorias.includes(a)?e.subcategorias=e.subcategorias.filter(t=>t!==a):e.subcategorias.push(a),d(l)}});else if(n===3){m.innerHTML=`
      ...
      <div style="display:flex; flex-direction:column; gap:12px; margin-bottom:16px;">
        ${[["☀️","Manhã","06h - 12h"],["☁️","Tarde","12h - 18h"],["🌙","Noite","18h - 22h"]].map(([o,a,t])=>`
          <div class="hora-card ${e.horarios.includes(a)?"active":""}" data-hora="${a}" style="
            display:flex; align-items:center; gap:16px; padding:16px; border-radius:16px; cursor:pointer; transition:all 0.2s;
            ${e.horarios.includes(a)?"background:#EEF2FF; border:1.5px solid #4F46E5;":"background:white; border:1px solid #D1D5DB;"}
          ">
            <span style="font-size:24px;">${o}</span>
            <div style="flex:1;">
              <p style="font-weight:600; font-size:15px; color:#111827; margin-bottom:2px;">${a}</p>
              <p style="font-size:13px; color:#64748B;">${t}</p>
            </div>
            <div style="width:20px; height:20px; border-radius:50%; display:flex; align-items:center; justify-content:center;
              ${e.horarios.includes(a)?"border:6px solid #4F46E5; background:white;":"border:2px solid #D1D5DB;"}
            "></div>
          </div>
        `).join("")}
      </div>
    `;const i=document.getElementById("inpRaio");i&&(i.oninput=o=>{e.raio=o.target.value,document.getElementById("raioDisplay").textContent=`${e.raio} km`}),document.querySelectorAll(".dia-chip").forEach(o=>{o.onclick=()=>{const a=o.dataset.dia;e.dias_semana.includes(a)?e.dias_semana=e.dias_semana.filter(t=>t!==a):e.dias_semana.push(a),d(l)}}),document.querySelectorAll(".hora-card").forEach(o=>{o.onclick=()=>{const a=o.dataset.hora;e.horarios.includes(a)?e.horarios=e.horarios.filter(t=>t!==a):e.horarios.push(a),d(l)}})}else if(n===4){m.innerHTML=`
    ...
    `;const i=document.getElementById("inpDoc"),o=document.getElementById("cardDoc");o&&i&&(o.onclick=()=>i.click(),i.onchange=r=>{const s=r.target.files[0];s&&(e.docFile=s,o.style.border="2px solid #10B981",o.style.backgroundColor="#ECFDF5",document.getElementById("statusDoc").innerHTML=`
            <div style="font-size:32px; margin-bottom:8px;">✅</div>
            <h3 style="font-size:16px; font-weight:600; color:#065F46;">Documento Anexado</h3>
            <p style="font-size:13px; color:#065F46;">${s.name}</p>
          `)});const a=document.getElementById("inpSelfie"),t=document.getElementById("cardSelfie");t&&a&&(t.onclick=()=>a.click(),a.onchange=r=>{const s=r.target.files[0];s&&(e.selfieFile=s,t.style.border="2px solid #10B981",t.style.backgroundColor="#ECFDF5",document.getElementById("statusSelfie").innerHTML=`
            <div style="font-size:32px; margin-bottom:8px;">✅</div>
            <h3 style="font-size:16px; font-weight:600; color:#065F46;">Selfie Anexada</h3>
            <p style="font-size:13px; color:#065F46;">${s.name}</p>
          `)})}document.getElementById("btnVoltar").onclick=()=>{n===1?u("splash"):(n--,d(l))},document.getElementById("btnAvancar").onclick=async()=>{var i;if(n===1){if(e.nome=document.getElementById("nome").value.trim(),e.email=document.getElementById("email").value.trim(),e.telefone=document.getElementById("telefone").value.trim(),e.bio=document.getElementById("bio").value.trim(),e.senha=document.getElementById("senha").value,!e.nome||!e.email||!e.senha){c("Preencha todos os campos obrigatórios","error");return}if(e.senha.length<6){c("Senha deve ter pelo menos 6 caracteres","error");return}n=2,d(l)}else if(n===2){if(!e.nicho){c("Selecione sua área de atuação","error");return}n=3,d(l)}else if(n===3){if(e.dias_semana.length===0){c("Selecione pelo menos um dia","error");return}if(e.horarios.length===0){c("Selecione pelo menos um horário","error");return}n=4,d(l)}else{const o=document.getElementById("btnAvancar");o.textContent="Enviando Documentos...",o.disabled=!0;try{const{supabase:a,uploadAvatar:t,uploadDocumentoKYC:r}=await x(async()=>{const{supabase:g,uploadAvatar:f,uploadDocumentoKYC:b}=await import("./index-DEW-c385.js").then(h=>h.i);return{supabase:g,uploadAvatar:f,uploadDocumentoKYC:b}},[]);await y({nome:e.nome,email:e.email,senha:e.senha,telefone:e.telefone,meta:{bio:e.bio,nicho:e.nicho,subcategorias:e.subcategorias,raio:e.raio,dias_semana:e.dias_semana,horarios:e.horarios}});const s=[];e.avatarFile&&s.push(t(e.avatarFile)),e.docFile&&s.push(r(e.docFile,"documento")),e.selfieFile&&s.push(r(e.selfieFile,"selfie")),s.length&&await Promise.all(s),u("splash",{aguardandoAprovacao:!0})}catch(a){console.error("ERRO CADASTRO PROFISSIONAL:",a),c((i=a.message)!=null&&i.includes("already")?"E-mail já cadastrado":"Erro ao criar conta. Veja o console.","error"),o.textContent="Enviar e Criar Conta",o.disabled=!1}}}}const p=()=>`
